...
class MyFirstGUI:
    def __init__(self, master):
        ...
        self.start_button.place(relx = 0.5, rely = 0.5, anchor = CENTER) #puts the greet button in the center (taken from http://stackoverflow.com/questions/18736465/how-to-center-tkinter-wiget)

        self.log_button = Button(master, text="Check the log", font = self.log_font, command=self.log_go, height = 1, width = 12)
        self.log_button.place(relx=0.95, rely=0.95, anchor=SE) #exit button in bottom right corner (taken from http://stackoverflow.com/questions/25396197/tkinter-put-a-widget-in-the-lower-right-corner-using-place)
